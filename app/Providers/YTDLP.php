...
class YTDLP
{
    ...
    public static function download(string $url, string $type)
    {
        ...
        foreach ($urls as $url) {
            ...
            $title = shell_exec($retrieveTitle);
            $title = trim($title);
            ...
            $coverFilepathTemp = $coverDirectory . $title . '_temp.png';
            $thumbnailBase = $coverDirectory . 'thumbnail';

            $artist = shell_exec($retrieveUploader);
            ...
            if (file_exists($downloadedThumbnail)) {
                ...
                rename($coverFilepathTemp, $coverFilepath);
            ...
            $media = new Media($filename, $title, $artist, $coverFilename, 'Unknown', 'Unknown', 'Unknown', 'None', $type, Auth::id());
